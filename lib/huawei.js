...
Router.prototype._prepareSession = async function() {
  ...
  if (res.statusCode != 200) {
    console.warn(`Unexpected SesTokInfo API response ${res.statusCode}: ${body}`);
    throw new Error(`Unexpected SesTokInfo API response code: ${res.statusCode}`);
  }

  const xmlRes = await xml.parse(body);
  this.COOKIE = `SessionId=${xmlRes('SesInfo').text()}`;
...
Router.prototype._request = async function (uri, data) {
  ...
  if (data != null) {
    ...
    if (this.opts.debug) console.debug(`Sending request: `, req);
    res = await http.post(req, data);
  }
  else {
    const req = {
      url: `http://${this.opts.ip}${uri}`,
  ...
  const body = (await res.body()).toString();
  if (res.statusCode !== 200) {
    console.warn(`Unexpected response from '${uri}' API with code ${res.statusCode}: ${body}`);
    throw new Error(`Unexpected response from '${uri}' API with code ${res.statusCode}`);
  }
  if (body.match(/<error>/)) {
    console.warn(`Found an XML error in the '${uri}' API response: ${body}`);
    throw new Error(`Found an XML error in the '${uri}' API response: ${body}`);
  }

  if (this.opts.debug) {
    console.debug("Got status: ", res.statusCode);
    console.debug("Got headers: ", res.headers);
...
Router.prototype.logout = async function() {
  const logoutXML = '<?xml version: "1.0" encoding="UTF-8"?><request><Logout>1</Logout></request>';

  // Login
  try {
    await this._requestWithAuth(`/api/user/logout`, logoutXML);
  }
  catch(ex) {
    throw ex;
  }
...
Router.prototype.getBasicInfo = async function() {
  ...
  const xmlRes = await xml.parse((await res.body()).toString());
...
Router.prototype.listSMS = async function(opts, page=1, count) {
  // Get SMS stats first
  if (!count) {
    const stats = await this.getSMSStats();
    count = stats.localinbox;
  }

  // List them
  const listSMSRequest = `<?xml version="1.0" encoding="UTF-8"?><request><PageIndex>${page}</PageIndex><ReadCount>${count}</ReadCount><BoxType>1</BoxType><SortType>0</SortType><Ascending>0</Ascending><UnreadPreferred>0</UnreadPreferred></request>`;
