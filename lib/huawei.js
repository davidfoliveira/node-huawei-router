...
Router.prototype._request = async function (uri, data) {
  ...
  let res;
  if (data != null) {
    console.log(`Posting to http://${this.opts.ip}${uri} : ${data}`);
    const req = {
    ...
    console.log(`Getting http://${this.opts.ip}${uri} ...`);
    res = await http.get({
      url: `http://${this.opts.ip}${uri}`,
      headers: {
        ...this.opts.headers,
...
Router.prototype._requestWithAuth = async function (uri, data) {
  ...
  while (attempts--) {
    try {
      ...
      console.log("Lixou: ", ex);
      // Should we reLogin?
      if (ex.toString().match(/<code>(100003|125002)<\/code>/) && opts.username && opts.password) {
        await this.login(this.opts.username, this.opts.password);
        continue;
      }
      throw ex;
...
Router.prototype.login = async function(username, password) {
  ...
  const loginRes = await this._request(`/api/user/login`, loginXML);
...
Router.prototype.getDeviceInfo = async function() {
  const res = await this._request('/api/device/information', null);
  const xmlRes = await xml.parse(res.body);
  return xmlToObj(xmlRes('response > *'));
};
...
Router.prototype.listSMS = async function(opts, count) {
  // Get SMS stats first
  if (!count) {
    const stats = await getSMSStats(opts);
